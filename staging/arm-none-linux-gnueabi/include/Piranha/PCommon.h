...
#define PUNUSED(x) (void)x
        ...
        #define PINLINE __attribute__((always_inline))
...
extern bool Asserts;                            ///< \brief Enables/Disables the asserts
...
#ifdef PRECISION_ASSERTS
            #define FASSERT(x, format, ...) if (!(x) && Asserts) do {   \
                 printf("ASSERT [%s:%d] ", __FILE__, __LINE__);         \
                 printf(format, ## __VA_ARGS__);                        \
                 printf("\n");                                          \
                 PTRAP;                                                 \
...
#else
            #define FASSERT(x, format, ...) do {} while (0)
#endif
    ...
    #if defined(IDEBUG) && defined(DO_PTRACE)
        #include <stdio.h>
