...
inline PFixed PFixed::operator*(const PFixed v) const
{
	...
	unsigned product;
	unsigned temp;
	DRUN(long long Perfect = ((long long)v.Value * (long long)Value) >> 16);
	PASSERT(DABS(Perfect) < 0x80000000, "Multiplication overflow");
	asm volatile (
			"smull %0, %1, %2, %3\n\t"
			"lsr %0, %0, #16\n\t"
			"orr %0, %0, %1, lsl #16\n\t"
	...
	PASSERT(DERROR((int)product, Perfect, 1), "Fixed* is not precise");
